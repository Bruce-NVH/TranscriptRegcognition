...
def findHorAndVerLines(image, rawImg):
    ...
    if lines is not None:
        for i in range(0, len(lines)):
            l = lines[i][0]
            if abs(l[3] - l[1]) < h_image / 60 and abs(
    ...
    if len(horizontal_lines_tmp) == 0:
        ...
        for i in range(0, len(vertical_lines_tmp)):
            ...
            if abs(l[3] - l[1]) >= avg_vertical_line * 0.99:
...
for i in range(0, len(horizontal_lines)):
    ...
    cv2.line(detectedLinesImg, (l[0], l[1]), (l[2], l[3]), (0, 0, 255), 3, cv2.LINE_AA)
for i in range(0, len(vertical_lines)):
    l = vertical_lines[i]
    cv2.line(detectedLinesImg, (l[0], l[1]), (l[2], l[3]), (0, 0, 255), 3, cv2.LINE_AA)
...
def findCropPoint(image, horizontalLines, verticalLines):
    ...
    if horizontalLines is None:
        ...
        cv2.line(image, (x1, y1), (x2, y1), (25, 183, 227), 3, cv2.LINE_AA)
        cv2.line(image, (x1, y2), (x2, y2), (25, 183, 227), 3, cv2.LINE_AA)
        cv2.line(image, (x1, y1), (x1, y2), (25, 183, 227), 3, cv2.LINE_AA)
        cv2.line(image, (x2, y1), (x2, y2), (25, 183, 227), 3, cv2.LINE_AA)
...
x1, x2, y1, y2 = findCropPoint(detectedLinesImg, horizontal_lines, vertical_lines)
...
for i in range(0, len(imgCropHorLines)):
    l = imgCropHorLines[i]
    cv2.line(detectedLinesImgCrop, (l[0], l[1]), (l[2], l[3]), (0, 0, 255), 3, cv2.LINE_AA)
